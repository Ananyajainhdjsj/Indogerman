...
def solve_circular_supply_chain_model(epsilon_limit, minimize_emissions_only=False):
    ...
    # --- OBJECTIVE ---
    Fixed_Cost = (quicksum(FixO[o]*W_o[o] for o in O) + quicksum(FixF[f]*W_f[f] for f in F) + quicksum(FixR[r]*W_r[r] for r in R))
    ...
    for c_src in C:
        for o in O:
            for k in K:
                # Collected from specific customer to collection must equal quicksum of routes out of that collected batch
                m.addConstr(
                    Y_cok[c_src, o, k] ==
    ...
    # Link aggregated outbound flows to the routed arc variables (aggregate = quicksum over source customers)
    for o in O:
        for c_dest in C:
            for k in K:
    ...
    for o in O:
        for k in K:
            total_collected = quicksum(Y_cok[c_src, o, k] for c_src in C)
            # Total reuse leaving collection o (to any customer)
            total_reuse = quicksum(Y_c_o_c2k[c_src, o, c_dest, k] for c_src in C for c_dest in C)
            ...
            total_refurb = quicksum(Y_c_o_fk[c_src, o, f, k] for c_src in C for f in F)
    ...
    for o in O:
        m.addConstr(quicksum(Y_cok[c, o, k] for c in C for k in K) * omega['Monocrystalline'] <= CAP_o[o] * W_o[o]) 
    for f in F:
        m.addConstr(quicksum(Y_ofk[o, f, k] for o in O for k in K) * omega['Monocrystalline'] <= CAP_f[f] * W_f[f])
    for r in R:
        m.addConstr(quicksum(Y_ork[o, r, k] * omega['Monocrystalline'] for o in O for k in K) <= CAP_r[r] * W_r[r])
    ...
    for c_src in C:
        for o in O:
            for k in K:
                ...
                m.addConstr(quicksum(Y_c_o_c2k[c_src, o, c_dest, k] for c_dest in C) <= Quality_Mix['Reuse_Cap'] * collected)
                # refurb from that collected batch limited to Refurb_Cap * collected
                m.addConstr(quicksum(Y_c_o_fk[c_src, o, f, k] for f in F) <= Quality_Mix['Refurb_Cap'] * collected)
    ...
    if m.status == GRB.OPTIMAL:
        ...
        for c in C:
            for o in O:
                val = quicksum(Y_cok[c, o, k].X for k in K)
                if val > tol:
                    print(f"{c} → {o}: {val:.1f}")
        ...
        for o in O:
            ...
            for c_dest in C:
                v = quicksum(Y_c_o_c2k[c_src, o, c_dest, k].X for c_src in C for k in K)
                if v > tol:
                    print(f"Reuse: {o} → {c_dest}: {v:.1f}")
            # Refurb aggregate per facility
            for f in F:
                v = quicksum(Y_c_o_fk[c_src, o, f, k].X for c_src in C for k in K)
                if v > tol:
                    print(f"Refurb: {o} → {f}: {v:.1f}")
            # Recycle
            for r in R:
                v = quicksum(Y_c_o_rk[c_src, o, r, k].X for c_src in C for k in K)
                if v > tol:
                    print(f"Recycle: {o} → {r}: {v:.1f}")
            # Landfill
            for l in L:
                v = quicksum(Y_c_o_lk[c_src, o, l, k].X for c_src in C for k in K)
                if v > tol:
        ...
        for f in F:
            for p in P:
                v = quicksum(Y_fpk[f, p, k].X for k in K)
                if v > tol:
                    print(f"{f} → {p}: {v:.1f}")

        print('\n=== PRODUCTION ===')
        for p in P:
            prod = quicksum(X_pk[p, k].X for k in K)
            print(f"{p} Production: {prod:.1f}")
            for c in C:
                shipped = quicksum(X_pck[p, c, k].X for k in K)
                if shipped > tol:
